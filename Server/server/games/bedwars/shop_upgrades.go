...
func (s *upgradesShop) Main() []item.Stack {
	...
	items[39] = getTrapItem(upgrades.Traps[2])
	items[40] = getTrapItem(upgrades.Traps[1])
	items[41] = getTrapItem(upgrades.Traps[0])
	return items
}

func (s *upgradesShop) uShopify(stack item.Stack, name, description string, tier int, costs []int) item.Stack {
	if len(costs)-1 >= tier {
